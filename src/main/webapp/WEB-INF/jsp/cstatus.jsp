...
<div class="x-nav">
      <span class="layui-breadcrumb">
        <a href="">首页</a>
        <a href="/cstatuslist">客户状态</a>
      </span>
    <a class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:right" href="/cstatuslist" title="刷新">
...
<div class="x-body">
    ...
    <xblock>
        ...
        <span class="x-right" style="line-height:40px">共有数据：${cs.totalCount} 条</span>
    </xblock>
    ...
    <table class="layui-table" >
        ...
        <tbody>
        <c:forEach items="${cs.list}" var="cs">
            <tr>
                <td style="display: none" class="id">${cs.id}</td>
                <td class="customerid">${cs.customerid}</td>
                <td class="nickname">${cs.nickname}</td>
                <td class="relname">${cs.relname}</td>
                <td class="createtime">${cs.createtime}</td>
                <td class="temperature">${cs.temperature}</td>
                <td class="isfever" >${cs.isfever}</td>
                <td class="isheadache" >${cs.isheadache}</td>
                <td class="isweakness" >${cs.isweakness}</td>
                <td class="issorethroat" >${cs.issorethroat}</td>
                <td class="isabdominalpain" >${cs.isabdominalpain}</td>
                <td class="td-manage">
                ...
                <td class="memo" style="display: none">${cs.memo}</td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
    ...
    <div class="layui-row" id="test" style="display: none; padding-top: 15px;padding-bottom: 20px;overflow-y: auto">
        <div class="layui-col" style="overflow-y: auto">
            <form class="layui-form" id="editCstatus" style="margin: 0 50px;overflow-y: auto">
                <fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px">
                    <legend id="editTitle">客户状态</legend>
                </fieldset>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" id="e_temperature" name="temperature" class="layui-input" placeholder="请输入体温">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">状态：</label>
                    <div class="layui-input-block">
                        <input type="checkbox" name="isfever" title="发热" value="是">
                        <input type="checkbox" name="isweakness" title="虚弱" value="是">
                        <input type="checkbox" name="isheadache" title="头痛" value="是">
                        <input type="checkbox" name="issorethroat" title="咽痛" value="是">
                        <input type="checkbox" name="isabdominalpain" title="腹痛" value="是">
                    </div>
                </div>
    ...
    <div class="" >
        <input type="hidden" id="totalPageCount" value="${cs.pageTotalCount}"/>
        <c:import url="pageBtn.jsp">
            <c:param name="totalCount" value="${cs.totalCount}"/>
            <c:param name="currentPageNo" value="${cs.pageIndex}"/>
            <c:param name="totalPageCount" value="${cs.pageTotalCount}"/>
        </c:import>
    </div>
</div>
<script>
    ...
    layui.use(['jquery', 'excel', 'form','layer','laydate','element'], function(){
        ...
        $("#addBtn").click(function () {
            document.getElementById("editCstatus").reset();
            document.getElementById("e_idinput").style.display="block";
            document.getElementById("editTitle").innerText = "客户状态";
            layer.open({
                type: 1,
                title: "添加客户状态信息",
                skin: "myclass",
